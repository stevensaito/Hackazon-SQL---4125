...
class Query extends DB\Query
{
    ...
    public function query() {
        ...
        $query = '';
        $params = array();    //I believe is what holds the user input
        if (!$this->parent) {
            $this->_aliases = [];
        }
        ...
        if ($this->_type == 'insert') {
            ...
            if (!empty($this->_conditions)) {
                $query .= "WHERE {$this->get_condition_query($this->_conditions, $params, true)} ";    /////////////////////////Look get_condition_query is just below
            }
		...
		///////////////////NEED TO FIND WHERE THE USERNAME AND PASSWORD ARE TAKEN IN
		//$login = mysqli_real_escape_string($this->request->post('username'));
        //$password = mysqli_real_escape_string($this->request->post('password'));
		/////////////////////THIS CAN HELP FILTER MORE
